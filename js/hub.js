...
G.scenes.hub = (() => {
  const STOPS = [
    { id: 'school', name: 'Okul Yolu', px: .12, py: .06, who: 'hacer', say: 'Günaydın Lina! Zil çalmadan okula yetiş, çantandaki eşyaları yolda topla.', tip: 'Zıplamak için ekrana dokun, kaymak için aşağı kaydır.' },
    { id: 'piano', name: 'Piyano', px: .29, py: .78, who: 'serkan', say: 'Piyano vakti! Notalar çizgiye geldiğinde tuşa dokun. Ben dans ederim.', tip: 'Tam zamanında dokun, kombo yap.' },
    { id: 'paint', name: 'Karalama Atölyesi', px: .46, py: .10, who: 'betul', say: 'Atölye açık! Fırçalar, simler ve damgalar seni bekliyor. İstediğini çiz.', tip: 'Bitirince kaydet, galerinde saklanır.' },
    { id: 'mall', name: 'AVM Turu', px: .63, py: .82, who: 'doruk', say: 'Lina! Viaport\'ta buluşalım, alışveriş listesini birlikte bitirelim!', tip: 'Listedeki eşyaları vitrinlerde bul.' },
    { id: 'sushi', name: 'Suşi Şefi', px: .79, py: .12, who: 'yunus', say: 'Bugün şef sensin! Herkesin siparişini hazırla, benimki bol somonlu olsun.', tip: 'Malzemeleri sırayla sürükle, sonra sar ve dilimle.' },
    { id: 'family', name: 'Aile Akşamı', px: .92, py: .84, who: 'serkan', say: 'Gün bitti, oyun zamanı! Rakibini seç, ekranı paylaşalım.', tip: 'İki kişilik: ekranın iki yarısı.' }
  ];
  let hair, cur = 0, walkTo = null, lx = 0, walkT = 0, ly = 0, bounce = 0, seagulls = [];
  const dayT = () => { const n = STOPS.filter(s => G.S.done[s.id]).length; return G.clamp(n / 6, 0, 1) * .92; };
  const stopXY = s => { const gy = G.H * .56; return { x: s.px * G.W, y: gy + 60 + s.py * (G.H - gy - 100) }; };
  function nextIdx() { const i = STOPS.findIndex(s => !G.S.done[s.id]); return i < 0 ? STOPS.length - 1 : i; }
  function intro(s) {
    if (!G.S.seen[s.id]) { G.S.seen[s.id] = 1; G.save(); }
    const p = G.panel(`<div class="who"><canvas id="av"></canvas><div><div class="name">${G.PEOPLE[s.who].name}</div><h2 style="font-size:28px">${s.name}</h2></div></div>
    ...
    p.querySelector('#av').replaceWith(Art.avatar(s.who, 84));
    p.querySelector('[data-a=x]').onclick = () => { G.closePanels(); Audio.sfx('tap'); };
    p.querySelector('[data-a=go]').onclick = () => { Audio.sfx('pop'); G.go(s.id); };
  }
  function dayEnd() {
    const lines = [
      `Bugün Lina ${G.S.best.school ? `okula <b>${G.S.done.school}★</b> ile yetişti` : 'okula gitti'}, piyanoda <b>${G.S.best.piano || 0}</b> puan yaptı`,
      `atölyede ${G.S.gallery.length} resim biriktirdi, <b>${G.PEOPLE.doruk.name}</b> ile AVM'yi gezdi`,
      `<b>${G.PEOPLE.yunus.name}</b>'a bol somonlu suşi yaptı ve akşam aileyle oyun oynadı.`
    ];
    const p = G.panel(`<h2>Gün tamamlandı!</h2><div class="diary">${lines.join(', ')}<br><br>Toplam yıldız: <b>★ ${G.S.totalStars}</b></div>
      <div class="actions"><button class="btn ghost" data-a="w">Gardırop</button><button class="btn coral" data-a="n">Yeni güne başla</button></div>`);
    p.querySelector('[data-a=w]').onclick = () => G.go('wardrobe');
  ...
  function drawStop(c, s, i, x, y, active, done) {
    ...
    switch (s.id) {
      ...
      case 'mall': const g = c.createLinearGradient(-80, -130, 80, 0); g.addColorStop(0, '#bfe3ff'); g.addColorStop(1, '#5fa8e0'); c.fillStyle = g; c.beginPath(); c.roundRect(-85, -130, 170, 130, [20, 20, 6, 6]); c.fill();
        ...
        c.fillStyle = O; c.beginPath(); c.roundRect(-70, -150, 140, 30, 8); c.fill(); G.txt(c, 'VIAPORT', 0, -135, 20, '#fff'); c.fillStyle = '#fff'; c.beginPath(); c.roundRect(-30, -40, 60, 40, [8, 8, 0, 0]); c.fill(); break;
      case 'sushi': c.fillStyle = '#2b2118'; c.beginPath(); c.roundRect(-70, -85, 140, 85, 8); c.fill(); c.fillStyle = '#d9534f'; c.fillRect(-78, -95, 156, 14); c.fillStyle = '#fbe3c0'; c.fillRect(-58, -70, 116, 40);
        ...
        c.font = '30px serif'; c.textAlign = 'center'; c.textBaseline = 'middle'; c.fillText('🍣', 0, -50); break;
  ...
  return {
    ...
    enter() { hair = hair || Art.makeHair(7); cur = nextIdx(); const p = stopXY(STOPS[cur]); lx = p.x; ly = p.y; walkTo = null; Audio.ambience(true);
      seagulls = Array.from({ length: 3 }, (_, i) => ({ x: Math.random() * G.W, y: 40 + i * 30, v: 25 + i * 8, s: .8 }));
      if (STOPS.every(s => G.S.done[s.id])) setTimeout(dayEnd, 600);
      else if (!G.S.seen.hub) { G.S.seen.hub = 1; G.save(); const pnl = G.panel(`<div class="who"><canvas id="av"></canvas><div><div class="name">Hacer</div><h2 style="font-size:28px">Hoş geldin Lina!</h2></div></div><p class="lead">“Bu Tuzla sahili. Her durakta bir macera var. Bir yere dokun, oraya yürü ve yıldız topla. Yıldızlarla gardıroptan yeni tokalar ve elbiseler alabilirsin.”</p><div class="actions"><button class="btn" id="ok">Hadi başlayalım</button></div>`); pnl.querySelector('#av').replaceWith(Art.avatar('hacer')); pnl.querySelector('#ok').onclick = () => { G.closePanels(); Audio.sfx('pop'); }; }
      // gardırop butonu
      const b = document.createElement('button'); b.className = 'chip'; b.textContent = '👗 Gardırop'; b.style.cssText = 'position:absolute;right:max(12px,env(safe-area-inset-right));bottom:max(14px,env(safe-area-inset-bottom));font-size:18px'; b.onclick = () => { Audio.sfx('tap'); G.go('wardrobe'); }; document.getElementById('ui').appendChild(b);
      const d = document.createElement('button'); d.className = 'chip'; d.textContent = `📔 Gün ${G.S.days + 1}`; d.style.cssText = 'position:absolute;left:max(12px,env(safe-area-inset-left));bottom:max(14px,env(safe-area-inset-bottom));font-size:18px'; d.onclick = () => { Audio.sfx('tap'); const n = STOPS.filter(s => G.S.done[s.id]).length; const p = G.panel(`<h2>Günlük</h2><div class="diary">Gün <b>${G.S.days + 1}</b> · ${n}/6 durak tamamlandı · bugün ★ ${Object.values(G.S.done).reduce((a, b) => a + b, 0)}<br>${STOPS.map(s => `${G.S.done[s.id] ? '✅' : '⬜️'} ${s.name}${G.S.best[s.id] ? ` <span style="color:#6b7a90">(en iyi ${G.S.best[s.id]})</span>` : ''}`).join('<br>')}${G.S.diary.length ? '<br><br>' + G.S.diary.slice(0, 5).join('<br>') : ''}</div><div class="actions"><button class="btn ghost" id="x">Kapat</button></div>`); p.querySelector('#x').onclick = () => G.closePanels(); }; document.getElementById('ui').appendChild(d);
    },
    update(dt) {
      for (const s of seagulls) { s.x += s.v * dt; if (s.x > G.W + 40) s.x = -40; }
      if (walkTo != null) { const tgt = stopXY(STOPS[walkTo]); const dx = tgt.x - lx, dy = tgt.y - ly, d = Math.hypot(dx, dy); const sp = 420 * dt; walkT += dt * 2.2;
        if (d < sp) { lx = tgt.x; ly = tgt.y; cur = walkTo; walkTo = null; Audio.sfx('pop'); intro(STOPS[cur]); } else { lx += dx / d * sp; ly += dy / d * sp; } }
    },
    down(p) { if (document.querySelector('#ui .panel')) return; // panel açıkken haritaya tıklama yok
    ...
    draw(c, W, H, T) {
      ...
      for (const s of seagulls) Art.seagull(c, s.x, s.y, T + s.x, s.s);
      ...
      // ağaçlar
      for (let i = 0; i < 9; i++) { const x = (i * 131 + 60) % W, y = gy + 40 + (i % 3) * 30; c.fillStyle = '#8b5a2b'; c.fillRect(x - 4, y - 30, 8, 30); c.fillStyle = i % 2 ? '#3f9e4a' : '#5cb35a'; c.beginPath(); c.arc(x, y - 42, 22, 0, 7); c.arc(x - 14, y - 30, 16, 0, 7); c.arc(x + 14, y - 30, 16, 0, 7); c.fill(); }
      // duraklar (y sırasına göre)
      const order = STOPS.map((s, i) => i).sort((a, b) => STOPS[a].py - STOPS[b].py);
      const drawn = new Set();
      for (const i of order) { const q = stopXY(STOPS[i]); if (ly < q.y - 20 && !drawn.has('lina') && i === order[order.length - 1]) { } drawStop(c, STOPS[i], i, q.x, q.y, i === cur && walkTo == null, G.S.done[STOPS[i].id]); }
      // Lina
