/* Açılış: şafak, deniz, Lina ve dalgalanan saçları */
G.scenes.title = (() => {
  let hair, t0, bird = [];
  return {
    landscape: false,
    enter() { hair = Art.makeHair(11); t0 = 0; bird = Array.from({ length: 4 }, (_, i) => ({ x: Math.random() * G.W, y: 60 + i * 40, s: .7 + Math.random() * .6, v: 30 + Math.random() * 30 }));
      Audio.ambience(true);
      const p = G.panel(`<div style="text-align:center">
        <p class="lead" style="color:#b6c6d8">Tuzla, İstanbul</p>
        ...
        ${G.S.totalStars > 0 ? `<p style="color:#b6c6d8;font-size:15px">Toplam ★ ${G.S.totalStars} · ${G.S.days} gün tamamlandı</p>` : ''}
      </div>`, 'dark bottom');
      p.style.width = 'min(520px,calc(100vw - 32px))'; if (G.W > 700) { p.style.left = 'auto'; p.style.right = '24px'; p.style.transform = 'none'; p.style.bottom = '50%'; p.style.translate = '0 50%'; }
      p.querySelector('#start').onclick = () => { Audio.unlock(); Audio.sfx('pop'); G.go('hub'); };
    },
    update(dt) { t0 += dt; for (const b of bird) { b.x += b.v * dt; if (b.x > G.W + 40) b.x = -40; } },
    draw(c, W, H, T) {
      const hz = H * .62; const t = .06 + Math.sin(T * .15) * .04;
      Art.sky(c, W, H, t, hz);
      Art.clouds(c, W, hz * .35, t, 10, .8);
      for (const b of bird) Art.seagull(c, b.x, b.y, T + b.x, b.s);
