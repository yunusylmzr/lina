...
G.scenes.family = (() => {
  const GAMES = ['color', 'count', 'sushi', 'math'];
  ...
  function makeCue() {
    ...
    } else if (roundGame === 'math') { // hızlı toplama
      const a = 2 + Math.floor(Math.random() * 9), b = 2 + Math.floor(Math.random() * 9), n = a + b;
      cue = { a, b, n, opts: [n, n + 1 + Math.floor(Math.random() * 2), Math.max(1, n - 1 - Math.floor(Math.random() * 2))].sort(() => Math.random() - .5) };
  ...
  function tap(side, p) {
    ...
    else if (roundGame === 'math') {
      const lx = side === 0 ? p.x : p.x - half(); const bw = half() / 3; const i = Math.floor(lx / bw);
      if (p.y > G.H - 120 && i >= 0 && i < 3) { if (cue.opts[i] === cue.n) { s[side]++; win(side, `${cue.a} + ${cue.b} = ${cue.n}`); } else { s[side] = Math.max(0, s[side] - 1); Audio.sfx('wrong'); } }
    }
  ...
  return {
    ...
    update(dt) { if (state === 'play') { roundT += dt;
      ...
      else if (roundGame === 'math') { if (roundT > 14) { state = 'between'; winner = -1; setTimeout(next, 800); } } } },
    draw(c, W, H, T) {
      const hw = half();
      // iki yarı
      ...
      if (state === 'ready') { G.txtShadow(c, `${round + 1}. tur`, W / 2, H / 2 - 40, 54, '#fff', 'rgba(0,0,0,.35)'); G.txt(c, roundGame === 'color' ? 'Turuncu görününce dokun!' : roundGame === 'count' ? 'Kaç balık var? Doğru sayıya dokun' : roundGame === 'math' ? 'Toplama yarışı: doğru cevaba dokun' : '10 saniye: suşileri yakala!', W / 2, H / 2 + 20, 26, '#1b1b2f'); }
      if (state === 'play' || state === 'between') {
        ...
        else if (roundGame === 'math') { for (const side of [0, 1]) { const cx = side * hw + hw / 2;
          c.save(); c.translate(cx, H / 2); if (side) c.rotate(Math.PI); G.txtShadow(c, `${cue.a} + ${cue.b} = ?`, 0, -30, Math.min(58, hw * .16), side ? '#1b5e8f' : '#d95a00', 'rgba(255,255,255,.7)'); c.restore();
          const bw = hw / 3; cue.opts.forEach((o, i) => { const x = side * hw + i * bw + bw / 2; c.fillStyle = side ? '#2e8bc0' : '#ff7a1a'; c.beginPath(); c.roundRect(x - bw / 2 + 12, H - 110, bw - 24, 90, 18); c.fill();
            c.save(); c.translate(x, H - 65); if (side) c.rotate(Math.PI); G.txt(c, o, 0, 0, 44, '#fff'); c.restore(); }); } }
